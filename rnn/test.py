...
from rnn.data_util import str2bool, PROJECT_DIR, N_LETTERS, N_GENDERS, name_to_tensor, ALL_GENDERS, NameGenderDataset, \
    name_gender_collate_cuda, VALSET, tensor_to_name, TESTSET, name_gender_collate, time_since
...
parser.add_argument('--weights', default='weights/old/gender_rnn_epoch47000.pth',
                    help='Weight state dict to load for testing')
...
def predict(name, n_predictions=2):
    output = _evaluate(name_to_tensor(name, args.cuda))
    ...
    while True:
        try:
            ...
            print("%.2f%% minibatch acc: %.4f (%s)" % (
                batch / (len(dataset) / batch_size), batch_acc / len(names_tensor), time_since(start)))
            cum += batch_acc
        except StopIteration:
            break
    acc = cum / len(dataset)
